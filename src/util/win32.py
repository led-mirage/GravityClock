...
def enable_high_dpi_awareness():
    """
    DPIスケーリングによる論理解像度を防ぎ、
    実際のピクセルサイズを取得できるようにする。
    （Windows 8.1 以降なら SetProcessDpiAwareness、
      それ以前なら SetProcessDPIAware を使用）
    """
    try:
        # Windows 8.1+（shcore.dllあり）
